...
page_numbers = [''] + [f'page-{i}/' for i in range(2, 3)]
        ...
        if len(script_tags) >= 4:
            ...
            if json_match:
                ...
                try:
                    ...
                    # rent_frequency = re.findall(r'"productLabel":\s*[^,]+,\s*"rentFrequency":\s*"([^"]+)"', fourth_last_script_content)
                    rent_frequency_pattern = re.findall(r'"rentFrequency":\s*"([^"]+)"',fourth_last_script_content)
                    print(rent_frequency_pattern)
